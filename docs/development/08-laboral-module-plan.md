...
| 3d — Automatizaciones simples | Sugerencias deterministas (nota de reunión → tarea) | **BLOCKED** — sigue sin una regla exacta definida por el Product Owner (¿qué dispara la sugerencia? ¿qué texto exacto? ¿se puede descartar?); no se adivinó para no inventar un requerimiento de negocio (CLAUDE.md) | Ninguna decidida todavía |
| 3e1 — Objetivos | Entidad `OBJECTIVE` independiente, progreso manual, resumen en "Hoy" | **TODO** — alcance definido (2026-08-22), listo para desarrollo. FR-031, UC-24, AC-018 | Ninguna — incremento aislado |
| 3e2 — Rutinas | Entidad `ROUTINE`, marcado manual de ejecución, **sin** generar `REMINDER`/`COMMITMENT` | **TODO** — alcance definido (2026-08-22), listo para desarrollo. FR-032, UC-25, AC-019 | Ninguna — deliberadamente separada de 3d |
| 3e3 — Lugares | Entidad `PLACE`, autocompletado de texto en `CreateTaskDialog` (sin FK en `REMINDER`) | **TODO** — alcance definido (2026-08-22), listo para desarrollo. FR-033, UC-26, AC-020 | Ninguna |
| 3e4 — Recursos | Entidad `RESOURCE`, vínculo opcional a Persona/Proyecto (no sustituye `DOCUMENT`) | **TODO** — alcance definido (2026-08-22), listo para desarrollo. FR-034, UC-27, AC-021 | Ninguna |
