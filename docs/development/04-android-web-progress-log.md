...
## 2026-08-16 — AND-007/WEB-007: notificaciones locales (gap real de V1, cerrado) + auditoría de seguridad

**Qué:** cierre de los dos gaps de V1 identificados en `docs/development/05-v2-plan.md` — notificaciones locales (nunca implementadas en ninguna plataforma) y un escaneo de secretos real (antes solo `grep` manual). Además, primera corrida real de la auditoría cruzada de seguridad acordada en la nota de `AND-006`/`WEB-006`.

**Plataformas:** Android (`AlarmManager` real), Web (`setTimeout` real, best-effort documentado).

**iOS:** no aplica todavía (desarrollo pausado) — cuando se retome, el patrón de Android (`UNUserNotificationCenter`/`UNNotificationRequest`) es el análogo real, no simulable con lo que hace Web.

**Bugs reales encontrados durante esta entrega:**

1. **Android: `POST_NOTIFICATIONS` nunca se solicitaba en tiempo de ejecución — afecta también a `AND-005`.** El permiso estaba en el manifest desde `AND-005` (push), pero ningún código de la app lo pedía en runtime. En API 33+ eso deja el permiso denegado para siempre sin que el usuario vea ningún diálogo — confirmado con `dumpsys notification` mostrando `com.vidacotidiana.app importance=NONE` pese a que la alarma programada disparó exactamente a tiempo (`dumpsys alarm`). Corregido para `AND-007` (se solicita junto con el permiso de alarma exacta, al activar "Add due date"); **`AND-005` probablemente tiene el mismo gap sin cerrar** — declarado explícitamente aquí, no corregido en silencio dentro de esta tarea.
   **iOS: riesgo no auditado.** `UNUserNotificationCenter.requestAuthorization` tendría que solicitarse de forma análoga cuando se implemente — no se ha verificado si el scaffold actual de `IOS-005` (pospuesto) ya lo contempla.

2. **Web, hallazgo del test, no de la app: `datetime-local` trunca a minuto.** Un primer test con margen de 6 segundos falló — el valor del input se trunca a su minuto, así que un margen tan corto podía quedar en el pasado para cuando el clic realmente se procesaba; `scheduleLocalReminder()` correctamente no programó nada (demora no positiva). El recordatorio se había creado bien, con la fecha correcta visible en la tarjeta — el bug era del margen elegido en el test, no de la app. Fix: margen de 130s truncado a minuto (≥70s reales de margen).
   **iOS: no aplica** (Web-specific).

**Verificación real:** Android — alarma real programada y disparada exactamente a tiempo (`dumpsys alarm`, `origWhen` coincide al milisegundo), notificación real confirmada en la bandeja de un dispositivo físico (captura de pantalla real + `dumpsys notification`), y **reinicio real del dispositivo** (`adb reboot`) con una alarma pendiente — tras el reinicio, `BootRescheduleReceiver` la reprogramó de verdad (confirmado en `dumpsys alarm` post-reinicio, mismo instante exacto) — esto solo es posible si el receiver corrió, ya que `AlarmManager` limpia toda alarma al reiniciar. Web — `e2e/local-notifications.spec.ts`: `new Notification(...)` real capturada envolviendo el constructor real vía `page.addInitScript()`, no mockeada. `sharing`/`notifications`/`error-tracking.spec.ts` (Web) y `LoginAndRemindersFlowTest`/`SharingFlowTest`/`NotificationsFlowTest` (Android) reverificados en verde tras los cambios. `./gradlew clean assembleDebug assembleRelease` y `npm run build`: ambos con éxito real.

**Auditoría de seguridad (`Documentacion/33-security-cross-audit.md`):** dos `DOCUMENTATION_CONFLICT` reales encontrados — `11-auth-security.md`/ADR-008 declaran protección contra fuerza bruta y recuperación de cuenta como resueltas por delegar en Keycloak, pero el realm real tiene `bruteForceProtected: false` y `resetPasswordAllowed: false`. Además, un gap no documentado antes: el autorregistro no funciona en ningún cliente (`registrationAllowed: false` en el realm, sin UI de registro en Android/Web) — corrige retroactivamente `05-v2-plan.md`, que había marcado ese criterio como `DONE` verificando solo login, no registro. Ninguno de los tres se corrigió aquí — quedan como decisión pendiente del Product Owner.
