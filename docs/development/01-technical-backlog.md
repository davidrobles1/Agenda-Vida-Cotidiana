...
**Nota sobre `BE-015` (alcance reducido, mismo día):** `openapi.yaml` describe `DELETE /reminders/{id}` como "cascades to INVITATION/REMINDER_SHARE and notifies active collaborators before deletion (DEC-002)". `REMINDER_SHARE`/`INVITATION` (`BE-016`..`021`) y el envío de push (`BE-025`) siguen `TODO` — no existen en el código todavía. Por lo tanto, hoy no hay nada que cascadear ni nadie que notificar: `BE-015` implementa el subconjunto del contrato que ya es válido con lo que existe (borrado del `REMINDER` propio, autorización por propiedad, `204`), no una versión incompleta de un requisito que sí pudiera cumplirse hoy. No es un pendiente oculto — es la consecuencia directa de que sharing/push todavía no están implementados, y se declara así explícitamente en vez de simularse. La cascada real a `INVITATION`/`REMINDER_SHARE` y la notificación a colaboradores activos (AC-013) se completan cuando se implementen sharing/notification — ya cubierto por `BE-022` (autorización `OWNER`/`COLLABORATOR`) y `BE-026` (eventos de push, incluyendo "eliminación") en este mismo backlog; no se abre un ID nuevo para esto.
