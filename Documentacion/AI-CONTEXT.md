...
- ADR-026 Relaciones entre registros: una garantía **exige** su artículo del inventario **en el alta**, validado en el servicio y no solo en la pantalla (dueño y módulo iguales); el mantenimiento se liga al artículo pero **opcionalmente** (también se mantiene lo que no es un objeto). Ningún vínculo obligatorio bloquea: el destino se crea en el mismo formulario. Nueva tabla `project_participants` (V32) que **convive** con `projects.client_person_id` como lista **disjunta** — `CLIENTE` no es un rol de participante, y el servicio impide el solapamiento por los dos lados. Ninguna regla es retroactiva (Accepted, 2026-09-06).
