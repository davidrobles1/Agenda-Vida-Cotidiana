...
**CERRADO (mecanismo), 2026-08-17 — `AND-009`, ver `01-technical-backlog.md` para el detalle completo.** `TokenStore` pasó a guardar el `AuthState` propio de AppAuth; `AuthManager.getValidAccessToken(forceRefresh)` usa `AuthState.performActionWithFreshTokens` (mecanismo documentado de AppAuth, no una llamada de refresh hecha a mano); `TokenAuthenticator` (nuevo `okhttp3.Authenticator`) reintenta una vez tras un 401 real antes de exigir login manual. El mecanismo servidor (`grant_type=refresh_token` contra el realm real) se confirmó por separado con una petición `curl` real: token nuevo, distinto del original, emitido con éxito. **No cerrado del todo — pendiente explícito, no oculto:** la verificación de punta a punta que pide la tarea (crear un recordatorio, esperar el vencimiento real de 5 minutos, crear otro sin volver a iniciar sesión) requiere el dispositivo físico del usuario y queda pospuesta hasta que lo indique — ver `05-v2-plan.md` §3.
