...
<@layout.registrationLayout displayMessage=!messagesPerField.existsError('username','password') displayInfo=realm.password && realm.registrationAllowed && !registrationDisabled??; section>
    ...
    <#elseif section = "socialProviders" >
        <#-- realm has no identityProviders configured (see realm-vida-cotidiana*.json)
             so social.providers is always empty for V1 — the per-provider icon
             switch from the stock template was dropped as dead code rather than
             copied for no reason; if a provider is added later, restore it from
             the base keycloak.v2 theme instead of re-guessing the SVGs. -->
